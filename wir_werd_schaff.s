...
load_image:
        ...
        # We did all prepare. Now beginn to work:
                ...
                li $v0, 14  		# 
                syscall             # read '5' and save in buffer at [1]

                addi $a1, $a1, 1    # move to next byte in file
            ...
            #### We got all that we need in header. 
            #### Convert data to int:
            ...
            # HURA WE REACH data
            read_data:
                bge $a1, $t8, end_read_data		# we check everytime if we reach the end of file.
                li $v0, 14
    ...
    ### We should be now almost done with load_img. 
    ### Close file:
        li $v0, 16             
        move $a1, $s0           
        syscall
...
###### ACTUNG: You are leaving the load_img!
    # $v0: breite
    # $v1: hoehe
    # $t9: adress where where save pixel data.
...
store_img:
            ...
            ###### 'space' #####
            li $t0, 0x20            # " "
            sb $t0, 0($s0)          # store space
            addi $s0, $s0, 1        # move forward

            ###### hoehe #####
            li $t1, 48              # 0 in decimal     
            li $t2, 57              # 9
            ...
            li $v0, 13          # Let't open output file:
            la $a0, fin_out     # a0 holds name file output
            li $a1, 1           # 0/1/9 = read/write/append
            li $a3, 0           # a3 = Mode  (we ignored. Don't know why but it works)
            syscall             # Open file. If file not available, it will create a file.

            move $t9, $v0       # $t9 holds descriptor
            li $t7, 20          # hard code 20 for header.
            mul $t8, $s1, $s2   # breite * hoehe = number of pixel
            add $t8, $t8, $t7   # now we know how many should we write from buffer_write

            # DO IT:
            li $v0, 15              # 15 : Write file
            move $a0, $t9           # $a0 = File descriptor
            la $a1, buffer_write    # $a1 = Writing Buffer
            move $a2, $t8           # $a2 = how many byte to write
            syscall
...
###### ACTUNG: You are leaving the store_img!
...
quantize:
	beq $ra, $zero, main	# wenn interpolate2 aus der main aufgerufen wird --> COMMIT	

    	la $a1, beispiel_bild	# beispiel_bild auf a1
    	
    	la $s0, beispiel_bild_width	#getting the address of width
    	lw $t3, 0($s0)			# get the value of the width
    	...
    	mul $t2, $t3, $t3		#berechnet länge vom array(Widh * height)
...
loop6:
	...
	lb $t3, ($t1)		#Die Zahl (word) wird geladen
	addi $t1, $t1, 1	# $t1 = $t1 + 4	// Fuer das naechste Wort!
	
	 
	div $t6, $t3, $s2	# $t6 = $t3 / x
	sb $t6, ($t7)		# quantiArray[] = $t6
	addi, $t7, $t7, 1	# $t7 = 4 + $t7
...
main:
    ...
    sw $s2, beispiel_bild_height
  
    
     #Zum Kontrollieren, wlche Speicheraddresse die Variablen haben
    la $t6, beispiel_bild
    la $t7, beispiel_bild_height
    #la $t4, beispiel_bild_width
    la $t3, array
