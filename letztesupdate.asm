...
#### 3==> this is new, 2:33 19.01
		read_breite:
			...
			li $t2, 48
			la $t9, buffer
			lb $t0, 0($t9)					# $t0 holds byte just read
			...
			sb $t0, 0($s3)				# we saved $t0 to the adress $s3 holds
		...
		read_hoehe:
			...
			sb $t0, 0($s4)					# load byte just read
...
#### Convert data to int:
		convert_breite:
			...
			lb	$t2, 0($t1)   			# $t2 holds first byte of buffer_breite
			...
			##			
			li $t9, 10
			lb	$t2, 1($t1)   			# $t2 holds 2nd byte of buffer_breite
			mult	$t0, $t1			# $t0 * $t1 = Hi and Lo registers
			mflo	$t4					# copy Lo to $t4
			add $t3, $t3, $t4			# $t3 holds result (for now 250). Don't TOUCH!
			##
			lb	$t2, 2($t1)   			# $t2 holds 3th byte of buffer_breite
		...
		convert_breite_2:
			...
			##
			lb	$t2, 1($t1)   			# $t2 holds 2th byte of buffer_breite
...
convert_hoehe:
			...
			lb	$t2, 0($t1)   			# $t2 holds first byte of buffer_hoehe
			...
			##			
			li $t9, 10
			lb	$t2, 1($t1)   			# $t2 holds 2nd byte of buffer_hoehe
			mult	$t0, $t1			# $t0 * $t1 = Hi and Lo registers
			mflo	$t4					# copy Lo to $t4
			add $t3, $t3, $t4			# $t3 holds result (for now 250). Don't TOUCH!
			##
			lb	$t2, 2($t1)   			# $t2 holds 3th byte of buffer_hoehe
			add $t3, $t3, $t2			# $t3 should be now 256
			## Now we save it into $ibreite
			sb $t3, 0($s3)		    #done
		...
		convert_hoehe_2:
			...
			mflo	$t3					# $t3 holds result (for now 60). Don't TOUCH!.    copy Lo to $t3
			##
			lb	$t2, 1($t1)   			# $t2 holds 2th byte of buffer_hoehe
			add $t3, $t3, $t2			# $t3 should be now 64
