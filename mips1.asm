...
addi $t2, $t2, 72		# $t2 wird 48 zugewiesen (32 bedeutet in ascii 'space')
addi $t3, $zero, 87
blt $t1, $t2, end_rd_breite
bgt $t1, $t3, end_rd_breite	# er vergleicht jezt die aus dem Buffer gelesene Zahl mit 32(also einem space), wenn er ein Space findet, dann i++
addi $t4, $zero, 0
...
addi $t0, $zero, 3		#i++
lb $t1, buffer($t0)		# buffer wird auf $t1 gespeichert
addi $t2, $zero, 48		# $t2 wird 48 zugewiesen (32 bedeutet in ascii 'space')
addi $t3, $zero, 57 
bgt $t1, $t3, end_rd_hoehe
blt $t1, $t2, end_rd_hoehe	# er vergleicht jezt die aus dem Buffer gelesene Zahl mit 32(also einem space), wenn er ein Space findet, dann i++
addi $t4, $zero, 0
sb $t1, buffer_hoehe($t5)
addi $t5, $t5, 1                #
...
la $a1, buffer_breite($t4)
lb  $t1, buffer_breite
addi $t5, $zero, 0      # $t5 hold integer value of rd_breite. for now 0
addi $t6, $zero, 2      # i
beq $t6, $t1, convert2b  # t4 = 2


la $t7, buffer_breite
lb $t1, 0($t7)             # erstes elem von buffer_breite
addi $t1, $t1, -48      # t1 = 2
mult $t1, $s1     # 2 * 100   = 200
mfhi $t1
#--
lb $t7, 1($t7)            # t8 = 5. elem von buffer_breite
addi $s1, $zero, 10
mult $t7, $s1       # t7 = 50
mfhi $s1
addi $v0, $v0, 1
#--        # v0 ++
lb $t8, buffer_breite		# 6
add $t5, $t1, $t7       # 250
add $t5, $t5, $t8       # 256 
####
sb $t5, ibreite 
...
convert2b:
la $t6, buffer_breite
lb $t1, 0($t6)   # erstes elem von buffer_breite
addi $t1, $t1, -48      # t1 = 6
addi $s3, $s3, 10
mult $t1, $s3      # 6 * 10   = 60
mfhi $v0
